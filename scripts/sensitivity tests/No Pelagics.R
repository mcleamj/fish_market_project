...
###################################################
###################################################
## SENSITIVITY TEST FOR REMOVING PELAGIC SPECIES ##
###################################################
###################################################
...
saveRDS(reef_metrics, "reef_metrics_no_pelagic.rds")

landings_metrics <- data.frame(PC1_centroid_landings, PC2_centroid_landings, landings_entropy, landings_di, landings_vuln)
saveRDS(landings_metrics, "landings_metrics_no_pelagic.rds")

######################################
## CORRELATION WITH ORIGINAL VALUES ##
######################################

reef_metrics_no_pelagic <- readRDS("reef_metrics_no_pelagic.rds")
reef_metrics_original <- readRDS("reef_metrics_original.rds")

landings_metrics_no_pelagic <- readRDS("landings_metrics_no_pelagic.rds")
landings_metrics_original <- readRDS("landings_metrics_original.rds")

all(rownames(reef_metrics_original) == rownames(reef_metrics_no_pelagic))
all(rownames(landings_metrics_no_pelagic) == rownames(landings_metrics_original))
...
range(reef_metrics_no_pelagic$reef_di)

range(reef_metrics_no_pelagic$reef_vuln)
...
range(landings_metrics_no_pelagic$landings_di)

range(landings_metrics_original$landings_vuln)
range(landings_metrics_no_pelagic$landings_vuln)
...
plot_data <- rbind(
  ...
  landings_metrics_no_pelagic %>%
    rownames_to_column("geographic") %>%
    mutate(source = "pelagic")
)
...
lims <- df_long %>%
  group_by(metric) %>%
  summarise(
    lo = min(c(original, pelagic), na.rm = TRUE),
    hi = max(c(original, pelagic), na.rm = TRUE),
    .groups = "drop"
  )

ggplot(df_long, aes(original, pelagic)) +
  geom_point(size = 3) +
...
diff_metrics <- data.frame(reef_metrics_no_pelagic - landings_metrics_no_pelagic) %>%
